...
class ML_models():
    ...
    def create_kfolds(self):
        ...
        hids=labels.iloc[:,0]
        y=labels.iloc[:,1]
        y.fillna(0, inplace=True)  # Remplacer les NaN par 0 dans Y_train
    ...
    def ml_train(self):
        ...
        for i in range(self.k_fold):
            ...
            #Affichage des données avant l'encodage
            print("les données gender X_train avant l'encodage:")
            print(X_train)
            print("les colonnes de X_train avant l'encodage:")
            print(X_train.columns)
            
            #encoding categorical
            gen_encoder = LabelEncoder() #OneHotEncoder(handle_unknown='ignore', sparse_output=False) #LabelEncoder()
            eth_encoder = LabelEncoder()#OneHotEncoder(handle_unknown='ignore', sparse_output=False) #LabelEncoder()
            ins_encoder = LabelEncoder()#OneHotEncoder(handle_unknown='ignore', sparse_output=False) #LabelEncoder()
            age_encoder = LabelEncoder()#OneHotEncoder(handle_unknown='ignore', sparse_output=False) #LabelEncoder()
            ...
            print('les données après l encodeage ')	
            ...
            # Affichage des premières lignes pour vérifier le résultat
            print("les données test avant l'encodeage: ")
            print(X_test)
